  ...
  function renderAnalyticsEditableItem(item, options = {}) {
    ...
    return `
        <div class="analytics-receipt-item" data-item-id="${escapeHtml(item.id)}">
          ...
          <button class="secondary analytics-save-item-btn" data-item-id="${escapeHtml(item.id)}" type="button" disabled>Save</button>
          <button class="secondary analytics-track-btn" data-item-id="${escapeHtml(item.id)}" data-item-name="${escapeHtml(item.item_name)}" data-item-amt="${item.line_total || 0}" type="button">Track</button>
        </div>`;
  }
