...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
  ...
  function isLikelyVariableWeightItem(item, measurement) {
    ...
    const variableCategories = new Set(["Fruit", "Vegetables", "Meat", "Seafood", "Deli & Chilled"]);
    ...
    return decimalQuantity || (variableCategories.has(category) && !commonPack);
  }
    ...
    if (!comparison.cheapest) {
      return `
        <div class="price-history-store-compare muted">
          ...
          <span>There are purchases from different shops, but not enough latest shop scans within 3 months of each other to call a cheapest shop.</span>
        </div>`;
    }

    const comparedStores = comparison.comparablePoints.map((point) => (
      `${point.store} ${formatCurrency(point.price)} on ${formatReceiptDate(point.date)}`
    )).join("; ");

    return `
      <div class="price-history-store-compare">
        <strong>Cheapest recent shop: ${escapeHtml(comparison.cheapest.store)}</strong>
        <span>${escapeHtml(formatCurrency(comparison.cheapest.price))} ${escapeHtml(priceUnit)} from ${escapeHtml(formatReceiptDate(comparison.cheapest.date))}. Compared latest scans from each shop within 3 months of ${escapeHtml(formatReceiptDate(comparison.newestDate))}: ${escapeHtml(comparedStores)}.</span>
      </div>`;
  }
    ...
    const rows = points.slice(-8).reverse().map((point) => `
      <div class="price-history-row${point.id === state.analyticsSelectedHistoryItemId ? " selected" : ""}">
        ...
        <span class="price-history-source">
          ...
          <small>Recorded qty: ${escapeHtml(point.quantity || "none")} - Line total: ${escapeHtml(formatCurrency(point.lineTotal))} - ${escapeHtml(point.basis)}</small>
        </span>
