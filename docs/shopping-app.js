...
(function () {
  ...
  async function createMembership(shopName, file) {
    if (!state.household) throw new Error("No household loaded.");
    if (!shopName) throw new Error("Add the shop name first.");
    if (!file) throw new Error("Choose a barcode picture first.");

    const safeName = getSafeMembershipFileName(file);
    const imagePath = `${state.household.id}/${Date.now()}-${safeName}`;
    ...
    const { error: insertError } = await state.client
      ...
      .insert({
        ...
        image_filename: file.name || safeName,
        created_by: state.session.user.id
      });
  ...
  function renderMemberships() {
    ...
    elements.membershipsEmptyState.querySelector(".message").textContent = "No membership barcodes yet. Add your first one above.";
    ...
    state.memberships.forEach((membership) => {
      ...
      meta.textContent = "Tap to show barcode";
      copy.appendChild(title);
      copy.appendChild(meta);
  ...
  function renderHouseholdState() {
    ...
    if (hasHousehold) {
      ...
      [elements.listPanel, elements.listsPane, elements.membershipsPane, elements.notesPane, elements.recipeBooksPane, elements.sidebarPanel].forEach((p) => {
  ...
  function getAnalyticsDateRange() {
    ...
    const range = state.analyticsRange || "current-month";
  ...
  async function saveAnalyticsReceiptItem(itemId, itemName, category) {
    ...
    state.receipts = state.receipts.map((receipt) => ({
      ...
      items: (receipt.items || []).map((item) => (
        item.id === itemId
          ? { ...item, item_name: nextName, category: nextCategory }
          : item
      ))
    }));
    return true;
  ...
  function renderAnalytics() {
    ...
    const receiptsHtml = receipts.map((r) => {
      ...
      return `
        <div class="analytics-receipt-row" data-receipt-id="${escapeHtml(r.id)}">
          ...
          ${r.image_path ? `<button class="secondary analytics-view-btn" data-receipt-id="${escapeHtml(r.id)}" type="button">View</button>` : "<span></span>"}
          <button class="secondary analytics-delete-btn" data-receipt-id="${escapeHtml(r.id)}" type="button">Delete</button>
        </div>
    ...
    elements.analyticsContent.querySelectorAll(".analytics-receipt-item").forEach((row) => {
      ...
      const updateSaveState = () => {
        const dirty = nameInput.value.trim() !== nameInput.dataset.originalValue || categorySelect.value !== categorySelect.dataset.originalValue;
      ...
      saveButton.addEventListener("click", async (e) => {
        ...
        try {
          const saved = await saveAnalyticsReceiptItem(row.dataset.itemId, nameInput.value, categorySelect.value);
          if (!saved) {
    ...
    state.trackedLists.forEach((list) => {
      ...
      if (!list.items.length) {
        ...
        list.items.forEach((item) => {
          ...
          date.textContent = formatReceiptDate(item.purchase_date);
  ...
  function wireMembershipEvents() {
    if (elements.membershipForm) {
      elements.membershipForm.addEventListener("submit", async (event) => {
        ...
        try {
          await createMembership(shopName, file);
