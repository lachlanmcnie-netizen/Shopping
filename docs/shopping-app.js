...
(function () {
  ...
  async function saveReceipt() {
    ...
    const { error: itemsErr } = await state.client.from("shopping_receipt_items").insert(
      includedItems.map((i) => ({ receipt_id: receipt.id, name: i.name, amount: i.amount || null, quantity: i.quantity || null, category: i.category || "Other" }))
    );
    if (itemsErr) throw itemsErr;

    const trackedListId = elements.receiptTrackListSelect?.value;
    if (trackedListId && starredItems.length) {
      const { data: savedItems } = await state.client.from("shopping_receipt_items").select("id, name").eq("receipt_id", receipt.id);
      const trackPayload = starredItems.map((i) => ({
        tracked_list_id: trackedListId,
        receipt_item_id: savedItems?.find((s) => s.name === i.name)?.id || null,
        name: i.name,
  ...
  async function loadReceipts() {
    ...
    if (receipts && receipts.length) {
      const { data: items } = await state.client
        ...
        .select("id, receipt_id, name, amount, quantity, category")
    ...
    const receiptsHtml = receipts.map((r) => {
      ...
      const itemsHtml = (r.items || []).map((item) => `
        <div class="analytics-receipt-item">
          <span class="analytics-receipt-item-name">${item.name}</span>
          <span class="tag" style="font-size:0.72rem;padding:3px 7px">${item.category || "Other"}</span>
          <span class="analytics-bar-amt">${formatCurrency(item.amount || 0)}</span>
          <button class="secondary analytics-track-btn" data-item-id="${item.id}" data-item-name="${item.name}" data-item-amt="${item.amount || 0}" type="button">★ Track</button>
        </div>`).join("");
      return `
  ...
  function parseIngredientLine(line) {
    ...
    if (qtyMatch) {
      ...
      const name = cleaned.slice(qtyMatch[0].length).trim();
      return name.length >= 2 ? { name, quantity } : { name: cleaned, quantity: "" };
    }
  ...
  async function loadRecipes() {
    ...
    const { data, error } = await state.client
      ...
      .select("id, name, ingredients, created_by, created_at")
      .eq("household_id", state.household.id)
      .order("created_at", { ascending: false });
    if (error) throw error;
    state.recipes = data || [];
  ...
  async function saveRecipe(name, ingredients) {
  ...
  function wireRecipeBooksEvents() {
    ...
    elements.recipeAddForm.addEventListener("submit", async (e) => {
      ...
      rows.forEach((row) => {
        ...
        if (!ingName) return;
        ingredients.push({ name: ingName, quantity: nameInput.dataset.quantity || "" });
      });

      if (!ingredients.length) { showToast("Select at least one ingredient."); return; }

      try {
        await saveRecipe(name, ingredients);
        await loadRecipes();
        renderRecipeBooks();
