...
(function () {
  ...
  function renderHouseholdState() {
    ...
    if (hasHousehold) {
      ...
      [elements.listPanel, elements.listsPane, elements.notesPane, elements.sidebarPanel].forEach((p) => {
  ...
  async function scanReceiptImage(file) {
    ...
    try {
      response = await fetch(appConfig.receiptScanFunctionUrl, {
        ...
        body: JSON.stringify({ imageDataUrl, imageName: state.receiptScan.imageName })
      });
    } catch (err) {
