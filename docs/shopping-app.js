...
(function () {
  ...
  async function saveReceipt() {
    ...
    const { error: itemsErr } = await state.client.from("shopping_receipt_items").insert(
      includedItems.map((i) => ({ receipt_id: receipt.id, item_name: i.name, line_total: i.amount || null, quantity: i.quantity || null, category: i.category || "Other" }))
    );
  ...
  function parseIngredientLine(line) {
    ...
    if (qtyMatch) {
      ...
      const rawName = cleaned.slice(qtyMatch[0].length).trim();
