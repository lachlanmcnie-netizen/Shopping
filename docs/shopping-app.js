...
(function () {
  ...
  function buildItemCard(item) {
    ...
    creator.textContent = getDisplayNameForUser(item.created_by);
    meta.appendChild(creator);

    if (item.purchased_at) {
      const purchased = document.createElement("span");
      purchased.className = "helper-text";
      purchased.textContent = new Date(item.purchased_at).toLocaleString([], {
        day: "numeric",
        month: "short",
        hour: "numeric",
        minute: "2-digit"
  ...
  async function subscribeRealtime() {
    ...
    state.channels.push(itemsChannel, notificationsChannel);
  }
