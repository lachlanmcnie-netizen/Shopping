...
alter table public.shopping_memberships add column if not exists membership_text text;
alter table public.shopping_memberships alter column image_path drop not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'shopping_memberships_has_info'
      and conrelid = 'public.shopping_memberships'::regclass
  ) then
    alter table public.shopping_memberships
      add constraint shopping_memberships_has_info
      check (image_path is not null or nullif(trim(membership_text), '') is not null);
  end if;
end $$;
